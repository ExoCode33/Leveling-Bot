 ...
 * COMPLETE VERSION with "Leveling-Bot" table prefix and guild settings support
 */
class DatabaseManager {
    ...
    async initializeTables() {
        try {
            ...
            // Guild settings table - for channel configuration and settings
            await this.db.query(`
    ...
    /**
     * Guild settings management - COMPLETE FUNCTIONS
     */
    ...
    async createDefaultGuildSettings(guildId) {
        try {
            const result = await this.db.query(`
                INSERT INTO ${this.tables.guildSettings} (guild_id)
                VALUES ($1)
                RETURNING *
            `, [guildId]);
    ...
    async updateGuildSetting(guildId, settingName, settingValue) {
        try {
            ...
            // Build dynamic query based on setting name
            const validSettings = [
                'levelup_channel', 'levelup_enabled', 'levelup_ping_user',
                'xp_log_channel', 'xp_log_enabled', 'xp_log_messages', 
                'xp_log_reactions', 'xp_log_voice', 'xp_log_levelup',
                'message_xp_min', 'message_xp_max', 'voice_xp_min', 'voice_xp_max',
                'reaction_xp_min', 'reaction_xp_max', 'daily_xp_cap', 'xp_multiplier'
            ];
    ...
    async getDatabaseStats() {
        try {
            ...
            const guildResult = await this.db.query(`SELECT COUNT(*) as total_guilds FROM ${this.tables.guildSettings}`);
